    var rule = {
        title: 'kk', // csp_AppYsV2
        host: 'http://ys.realdou.cn:16285',
        ...
        timeout: 5000,
        class_name: '鐢靛奖&鐢佃鍓�&鍔ㄦ极&缁艰壓', // 鍒嗙被绛涢€� /api.php/app/nav
        class_url: '1&2&3&4',
        play_parse: true,
        lazy:'js:if(/m3u8|mp4/.test(input)){input}else{let purl=request("http://103.73.163.111/zy.php?url="+input);input={jx:0,url:JSON.parse(purl).url,parse:0}}',
        limit: 6,
        鎺ㄨ崘: 'json:list[0].vlist;*;*;*;*',
        涓€绾�: 'json:list;vod_name;vod_pic;vod_remarks||vod_score;vod_id',
        浜岀骇: 'js:try{let html=request(input);print(html);html=JSON.parse(html);let node=html.data;VOD={vod_id:node["vod_id"],vod_name:node["vod_name"],vod_pic:node["vod_pic"],type_name:node["vod_class"],vod_year:node["vod_year"],vod_area:node["vod_area"],vod_remarks:node["vod_remarks"],vod_actor:node["vod_actor"],vod_director:node["vod_director"],vod_content:node["vod_content"].strip()};let episodes=node.vod_url_with_player;let playMap={};if(typeof play_url==="undefined"){var play_url=""}episodes.forEach(function(ep){let source=ep["name"];if(!playMap.hasOwnProperty(source)){playMap[source]=[]}playMap[source].append(ep["url"])});let playFrom=[];let playList=[];Object.keys(playMap).forEach(function(key){playFrom.append(key);playList.append(playMap[key])});let vod_play_from = playFrom.join("$$$");let vod_play_url=playList.join("$$$");VOD["vod_play_from"]=vod_play_from;VOD["vod_play_url"]=vod_play_url}catch(e){log("鑾峰彇浜岀骇璇︽儏椤靛彂鐢熼敊璇�:"+e.message)}',
        鎼滅储: '*',
    }